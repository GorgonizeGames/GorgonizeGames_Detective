...
{
    public class SteamSaveService : MonoBehaviour, ISaveService
    {
        [Header("Settings")]
        ...
        public async Task<bool> SaveDataAsync<T>(string key, T data) where T : class
        {
            ...
            try
            {
                ...
                if (_steamInitialized && IsCloudEnabled)
                {
                    ...
                    if (success)
                    {
                        ...
                        _eventService?.Publish(new GameSavedEvent { SaveSlot = key });
                        return true;
                    }
            ...
            {
                Debug.LogError($"[SteamSaveService] Save failed: {e.Message}");
                OnSaveCompleted?.Invoke(key, false);
                return false;
            }
        }
        ...
        public async Task<T> LoadDataAsync<T>(string key) where T : class
        {
            ...
            try
            {
                ...
                if (_steamInitialized && IsCloudEnabled && SteamRemoteStorage.FileExists(cloudPath))
                {
                    ...
                    if (readBytes > 0)
                    {
                        ...
                        _cache[key] = data;
                        OnLoadCompleted?.Invoke(key, true);
                        _eventService?.Publish(new GameLoadedEvent { SaveSlot = key });
                        return data;
                    }
                }
#endif
            ...
            {
                Debug.LogError($"[SteamSaveService] Load failed: {e.Message}");
                OnLoadCompleted?.Invoke(key, false);
                return null;
            }
        }
        ...
        public async Task<bool> DeleteDataAsync(string key)
        {
            ...
            {
                Debug.LogError($"[SteamSaveService] Delete failed: {e.Message}");
                return false;
            }
        }
        ...
        public async Task<bool> SyncWithCloudAsync()
        {
            ...
            {
                Debug.LogError($"[SteamSaveService] Sync failed: {e.Message}");
        ...
        public List<SaveSlotInfo> GetAllSaveSlots()
        {
            ...
            {
                Debug.LogError($"[SteamSaveService] Error getting slots: {e.Message}");
            }
#endif
